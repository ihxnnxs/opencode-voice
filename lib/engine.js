...
function repairWavHeader(file) {
  ...
  try {
    ...
    if (probe.toString("ascii", 0, 4) !== "RIFF" || probe.toString("ascii", 8, 12) !== "WAVE") return;
    ...
    while (offset + 8 <= stat.size) {
      ...
      if (id === "fmt " || id === "fact" || id === "LIST" || id === "bext" || id === "cue " || id === "smpl" || id === "PAD " || id === "JUNK") {
    ...
    const actual = Math.min(dataSize, stat.size - dataStart);
    const patch = Buffer.alloc(8);
    patch.writeUInt32LE(stat.size - 8, 0);
    patch.writeUInt32LE(actual, 4);
    fs.writeSync(fd, patch.subarray(0, 4), 0, 4, 4);
    fs.writeSync(fd, patch.subarray(4), 0, 4, dataStart - 4);
  } catch {
    // best-effort; never fail transcription because of a repair issue
  } finally {
    try {
      fs.closeSync(fd);
    } catch {}
...
export class VoiceRuntime {
  ...
  async transcribe(audioFile, model, settings = {}) {
    ...
    const args = model.engine === "transcribe-cpp" ? ["--model", modelFile, "--file", audioFile] : ["-m", modelFile, "-f", audioFile, "-np", "-nt"];
